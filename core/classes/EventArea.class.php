...
class EventArea {
    ...
    public function html() {
        ...
        $t_time_above      = $t_event_timestamp - Column::$time_period_list[0];

        $t_top   = (( $t_time_above / 60 ) / (Column::$intervals_per_hour )) * Column::$html_interval_height;
        $t_left  = ( 100 / $this->total_event_in_group ) * $this->current_number_in_group;
        $t_hight = ( ( $this->event['duration'] / 60 ) / (Column::$intervals_per_hour ) ) * Column::$html_interval_height;
        $t_width = ( 100 - $t_left - 5);
//        $t_width = 100 / $this->total_event_in_group;
